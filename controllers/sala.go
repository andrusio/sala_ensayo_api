...
func GetSalaGrupo(c *gin.Context) {
	fecha := c.Query("fecha")
	sqlstm := fmt.Sprintf("SELECT sg.id, s.nombre, g.nombre, sg.hora_desde, sg.hora_hasta FROM sala_grupo sg "+
	...
	for results.Next() {
		...
		var err error
		err = results.Scan(&salaGrupo.ID, &salaGrupo.Sala, &salaGrupo.Grupo, &salaGrupo.HoraDesde, &salaGrupo.HoraHasta)
		if err != nil {
			panic("Error al validar datos " + err.Error())
		}
