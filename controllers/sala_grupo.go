...
func GetSalaGrupo(c *gin.Context) {
	...
	db := sqldb.ConnectDB()
	res, err := db.Query(`SELECT sg.id, s.nombre sala, s.id sala_id, s.color sala_color, g.nombre grupo, g.id grupo_id, sg.hora_desde, sg.hora_hasta 
		FROM sala_grupo sg 
	...
	for res.Next() {
		...
		var err error
		err = res.Scan(&salaGrupo.ID, &salaGrupo.Sala, &salaGrupo.SalaId, &salaGrupo.SalaColor, &salaGrupo.Grupo, &salaGrupo.GrupoId, &salaGrupo.HoraDesde, &salaGrupo.HoraHasta)
		if err != nil {
